...
async function initDatabase() {
  try {
    ...
    await mongoose.connection.dropCollection(
        'ads',
        function() {
          console.log('Collection ads dropped');
        },
    );
    ...
    await mongoose.connection.dropCollection(
      'tags',
      function() {
        console.log('Collection tags dropped');
      },
    );
    ...
    Tag.create(newTags).then((result) => {
      console.log('Added sample tags: ' + result.map(e => e.tag));
    }).catch((err) => {
      console.log('Error adding data', err);
    });
    
    Ad.create(newAds).then((result) => {
      console.log('Added sample ads: ' + result.map(e => e.title))
    }).catch((err) => {
      console.log('Error adding data', err);
    });
    ...
    User.create({ 
      ...
      console.log('Added sample user:', result.email);
